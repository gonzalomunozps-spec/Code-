...
| `grados_dia.py` | Integrales térmicas (grados-día). Separa dos conceptos: el **cero vegetativo** (Tª base, propio de cada especie, `CERO_VEGETATIVO` — se autorrellena en el alta y es editable) y el **método de cálculo** (`METODOS_CALCULO`: los cuatro del temario — directo/Reaumur, tiempo térmico/residual, exponencial y heliotérmico, este último con `horas_luz` por el modelo CBM). Acumula desde el clima diario, tiene hitos de GDD por fase y `fase_override` (con integral definida, la fase del extensivo la marca el GDD). Los hitos están en °C·día de tiempo térmico: con otro método el resumen avisa de que la fase es orientativa. `params_integral` lee tanto el formato nuevo como el antiguo (`metodo:'base6'`). Núcleo puro; el clima, perezoso a `clima_era5`/`almacen`. **Opcional.** |
...
Comprobado borrando cada fichero: completo 740, sin `validacion` 718,
