...
# =====================================================================
# 6. MITIGACION DEL EFECTO CUBIERTA: NDVI DE COPA (desmezclado)
# =====================================================================
"""
Problema: en un lenoso, el NDVI medio de la parcela mezcla la senal de la COPA
con la del SUELO/CUBIERTA entre calles. Un olivar mediocre con cubierta verde
puede dar un NDVI mas alto que uno sano con suelo limpio -> el estado sale al reves.

Solucion (modelo de mezcla lineal de dos componentes):

    NDVI_parcela = fc * NDVI_copa + (1 - fc) * NDVI_fondo

donde:
    fc        = fraccion de cobertura de copa (la parte del pixel ocupada por arbol)
    NDVI_fondo= NDVI del suelo/cubierta entre calles (varia: desnudo ~0.15, hierba ~0.6)

Despejando:                NDVI_copa = (NDVI_parcela - (1-fc)*NDVI_fondo) / fc

Como estimamos cada termino sin datos de campo:

  * fc: se deduce de la MODALIDAD (marco de plantacion). Son valores tipicos de
    bibliografia para olivar; se pueden calibrar por finca.
  * NDVI_fondo: se estima con el MINIMO ESTIVAL de la propia parcela. En julio-agosto
    la cubierta esta agostada y la copa perenne sigue verde, asi que en esa epoca el
    fondo es suelo desnudo. Ese suelo es la linea base; cuando en primavera el NDVI
    sube por encima de lo que explica la copa, ese exceso es la cubierta.

Limitaciones (honestas):
  - Es una aproximacion, no una medida. Con fc mal estimada el resultado se degrada.
  - Solo aplica a LENOSOS. En extensivos el cultivo ES la cubierta.
  - Si la cubierta es muy densa y alta, puede tapar parcialmente la copa (no lineal).
"""

# Fraccion de cobertura de copa tipica por modalidad (calibrable por finca)
FC_COPA = {
    "TRADICIONAL":    0.25,   # marcos amplios, mucho suelo visible
    "INTENSIVO":      0.45,
    "SUPERINTENSIVO": 0.70,   # seto casi continuo
}

NDVI_SUELO_DESNUDO = 0.15     # valor de referencia si no hay historico estival


def ndvi_fondo_estimado(serie, fecha_iso):
    """
    Estima el NDVI del suelo/cubierta bajo la copa.
    Usa el minimo del periodo estival (jul-sep) de la propia parcela: en esa epoca
    la cubierta esta seca, asi que representa el suelo desnudo real de esa finca.
    """
    estivales = []
    for r in serie:
        f = r.get("fecha")
        ...
        if not f or v is None:
            continue
        mes = datetime.strptime(f, "%Y-%m-%d").month
        if mes in (7, 8, 9):
            estivales.append(v)
    if not estivales:
        return NDVI_SUELO_DESNUDO, "referencia generica (sin historico estival)"
    # el minimo estival contiene copa + suelo seco; el suelo es la parte baja
    return round(min(estivales) * 0.55, 3), "minimo estival de la propia parcela"


def ndvi_copa(tipo, subtipo, serie, fecha_iso=None):
    """
    Mitiga el efecto de la cubierta sobre el NDVI, devolviendo un NDVI CORREGIDO
    que representa el vigor atribuible a la COPA, EN LA MISMA ESCALA que el NDVI
    observado (para poder compararlo con los umbrales de fase).

    En vez de dividir por fc (que amplifica el ruido cuando la copa es pequena),
    se RESTA la contribucion de la cubierta:

        aporte_cubierta = (1 - fc) * (ndvi_fondo - suelo_desnudo)
        ndvi_corregido  = ndvi_observado - aporte_cubierta

    Es decir: se descuenta el verde extra que aporta la hierba entre calles
    respecto a lo que aportaria el suelo desnudo de esa misma finca.
    """
    if tipo != "LENOSO" or not serie:
        return None

    act = serie[-1]
    ndvi_obs = act.get("ndvi")
    if ndvi_obs is None:
        return None

    fecha = fecha_iso or act.get("fecha")
    fc = FC_COPA.get(subtipo, 0.45)
    suelo, origen = ndvi_fondo_estimado(serie, fecha)   # suelo desnudo de la finca

    cub = detectar_cubierta(tipo, subtipo, serie, fecha)
    brecha = (cub or {}).get("brecha_suelo_ndvi_msavi") or 0.0
    señales = (cub or {}).get("señales", 0)

    activa = señales >= 2 and brecha > 0.08

    if activa:
        # el fondo pasa de suelo desnudo a hierba verde; la brecha NDVI-MSAVI
        # mide el verde que NO esta en la copa
        fondo = min(0.75, suelo + brecha * 1.6)
        aporte = (1 - fc) * max(0.0, fondo - suelo)     # verde extra de la cubierta
    else:
        fondo, aporte = suelo, 0.0

    corregido = max(0.0, min(1.0, ndvi_obs - aporte))
    sesgo = round(ndvi_obs - corregido, 3)              # >0: la cubierta inflaba

    return {
        "ndvi_observado": round(ndvi_obs, 3),
        "ndvi_copa": round(corregido, 3),      # NDVI corregido, misma escala
        "sesgo_cubierta": sesgo,
        "aporte_cubierta": round(aporte, 3),
        "fc_copa": fc,
        "ndvi_fondo": round(fondo, 3),
        "ndvi_suelo_desnudo": suelo,
        "origen_fondo": origen,
        "cubierta_activa": activa,
        "explicacion": (
            f"Cubierta activa: el fondo entre calles ({fondo:.2f}) esta mas verde que el suelo "
            f"desnudo de la finca ({suelo:.2f}). Con una copa que cubre ~{int(fc*100)} %, la "
            f"cubierta infla el NDVI en {aporte:.3f} puntos. NDVI corregido de copa: "
            f"{corregido:.3f} (observado {ndvi_obs:.3f})."
            if activa else
            f"Sin cubierta activa: el NDVI observado ({ndvi_obs:.3f}) refleja ya el vigor de la copa."
        ),
    }
